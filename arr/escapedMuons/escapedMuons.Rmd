...
```{r, message=FALSE}
jobs <- lapply(1:length(arrFiles), function(i) paste0(scenarios[i], "_volNames.csv") %>% read_csv(col_names=c("volID", "volName")) %>% mcparallel(name=i) )
volNames <- mccollect(jobs)
```
...
OK - now I need to split up the data frame and apply these names. Fun!
```{r}
