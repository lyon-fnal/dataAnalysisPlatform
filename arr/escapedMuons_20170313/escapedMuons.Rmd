...
This file was an attempt to examine "ARR" (Accelerator Readiness Review) files and plot muon "escapees" for different scenarios (different magnetic fields are turned off). Instead, this turned into a sanity analysis when I didn't find any muons that decayed. Instead, they were all eaten by the iron yoke. The yoke was recently added by Nathan. Apparently there was a mistake (I think he rotated everything by 6 degrees the wrong way). 

So much of this document is figuring out that there were no muons stored. Then Nathan released some new code and I made a new 1000 events. For that file, I see 1% of muons stored. Still a little low, but Renee thinks it is fine (the gun still needs to be optimized). At the end of the file, we can see nice oscillation of a muon. 
...
Let's look at the volume names and see how many of these muons decay. It should be around 10% or so. 
